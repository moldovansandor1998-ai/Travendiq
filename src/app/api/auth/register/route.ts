...
export async function POST(req: NextRequest) {
  ...
  if (!EMAIL_RE.test(email) || name.length < 2 || password.length < 8 || password.length > 72) return NextResponse.json({ error: "invalid_input" }, { status: 400 });
  if (!(await rateLimit(svc, emailKey, 3, 900))) return NextResponse.json({ error: "rate_limited" }, { status: 429 });

  const { data, error } = await svc.auth.admin.generateLink({ type: "signup", email, password, options: { data: { full_name: name, locale, registration_intent: "provider" } } });
  if (error || !data.properties?.hashed_token || !data.user?.id) {
    ...
    if (message.includes("weak") || message.includes("password")) return NextResponse.json({ error: "weak_password" }, { status: 400 });
    if (message.includes("already") || message.includes("registered") || message.includes("exists")) return NextResponse.json({ error: "account_exists" }, { status: 409 });
